...
export const addItemsToInventory = (currentInventory, inventorySlots, itemsToAdd) => {
    ...
    const currentEquipmentCount = tempInventory.filter((item) => item.type === 'equipment').length;
    const equipmentCapacity = Number(inventorySlots.equipment);
    if (isNaN(equipmentCapacity) || itemsByType.equipment.length > (equipmentCapacity - currentEquipmentCount)) {
    ...
    for (const category of ['consumable', 'material']) {
        const items = itemsByType[category];
        if (items.length === 0)
            continue;
        const currentCategoryItems = tempInventory.filter((item) => item.type === category);
        let currentCategorySlotsUsed = currentCategoryItems.length;
        const stackableToAdd = {};
        ...
        for (const item of items) {
            ...
            while (quantityLeft > 0) {
                ...
                const template = [...Object.values(CONSUMABLE_ITEMS), ...Object.values(MATERIAL_ITEMS)].find(t => t.name === item.name);
                if (template) {
...
export const createItemInstancesFromReward = (itemRefs) => {
    const createdItems = [];
