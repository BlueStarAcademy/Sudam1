...
export const isDifferentDayKST = (ts1, ts2) => {
    if (!ts1 || ts1 === 0)
        return true;
    return !isSameDayKST(ts1, ts2);
};
export const isDifferentWeekKST = (ts1, ts2) => {
    if (!ts1 || ts1 === 0)
        return true; // Treat no previous update as a new week
    const d1 = getKSTDate(ts1);
    const d2 = getKSTDate(ts2);
...
export const isDifferentMonthKST = (ts1, ts2) => {
    if (!ts1 || ts1 === 0)
        return true;
    const d1 = getKSTDate(ts1);
    const d2 = getKSTDate(ts2);
...
export const getCurrentSeason = (date = Date.now()) => {
    const d = getKSTDate(date);
    const year = d.getUTCFullYear();
    const month = d.getUTCMonth(); // 0-11
    let season;
    if (month < 3)
        season = 1; // Jan, Feb, Mar (Month 0, 1, 2)
    else if (month < 6)
        season = 2; // Apr, May, Jun (Month 3, 4, 5)
    else if (month < 9)
        season = 3; // Jul, Aug, Sep (Month 6, 7, 8)
    else
        season = 4; // Oct, Nov, Dec (Month 9, 10, 11)
    const shortYear = year.toString().slice(-2);
    return { year, season, name: `${shortYear}-${season}시즌` };
};
export const getPreviousSeason = (date = Date.now()) => {
    const d = getKSTDate(date);
