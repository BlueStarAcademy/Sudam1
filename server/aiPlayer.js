...
import { KATAGO_LEVEL_TO_MAX_VISITS, SINGLE_PLAYER_STAGES } from '../constants.js';
...
export const getAiUser = (mode) => {
    return {
        ...baseAiUser,
        nickname: aiNicknames[mode] || 'AI 봇',
    };
};
const makeSimpleCaptureAiMove = (game) => {
...
const makeStrategicAiMove = async (game) => {
    ...
    for (const recommendedMove of recommendedMoves) {
        ...
        const validationResult = processMove(game.boardState, moveAttempt, game.koInfo, game.moveHistory.length);
...
const makeDiceGoAiMove = (game) => { };
const makeOmokAiMove = (game) => { };
const makeThiefAiMove = (game) => { };
const makeAlkkagiAiMove = (game) => { };
const makeCurlingAiMove = (game) => { };
export const makeAiMove = async (game) => {
