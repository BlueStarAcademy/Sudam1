...
import { handleAction, resetAndGenerateQuests } from './gameActions.js';
...
import { PLAYFUL_GAME_MODES, SPECIAL_GAME_MODES, SINGLE_PLAYER_MISSIONS, GRADE_LEVEL_REQUIREMENTS } from '../constants.js';
import { createDefaultBaseStats, createDefaultUser } from './initialData.js';
import { containsProfanity } from '../profanity.js';
const processSinglePlayerMissions = (user) => {
    const now = Date.now();
...
const startServer = async () => {
        ...
        try {
            ...
            for (const user of allUsers) {
                ...
                for (const type of tournamentTypes) {
                    const key = `last${type.charAt(0).toUpperCase() + type.slice(1)}Tournament`;
                    const tournamentState = updatedUser[key];
                        ...
                        if (!activeGame.disconnectionState) {
                            ...
                            if (activeGame.disconnectionCounts[userId] >= 3) {
                                ...
                                if (activeGame.moveHistory.length < 10) {
                                    ...
                                    if (!activeGame.canRequestNoContest)
                                        activeGame.canRequestNoContest = {};
            ...
            for (const negId of Object.keys(volatileState.negotiations)) {
                ...
                if (now > neg.deadline) {
                    ...
                    if (challengerStatus?.status === 'negotiating') {
                        ...
                        const hasOtherNegotiations = Object.values(volatileState.negotiations).some(otherNeg => otherNeg.id !== negId && otherNeg.challenger.id === challengerId);
            ...
            for (const game of updatedGames) {
                if (game.isAiGame || game.gameStatus === 'ended' || game.gameStatus === 'no_contest' || game.disconnectionState)
                    continue;
            ...
            for (const game of endedGames) {
                ...
                if (!isAnyoneInRoom) {
                    ...
                    const isRematchBeingNegotiated = Object.values(volatileState.negotiations).some(neg => neg.rematchOfGameId === game.id);
    ...
    app.post('/api/auth/register', async (req, res) => {
        try {
            ...
            if (nickname.trim().length < 2 || nickname.trim().length > 12)
                return res.status(400).json({ message: '닉네임은 2자 이상 12자 이하여야 합니다.' });
            if (containsProfanity(username) || containsProfanity(nickname))
                return res.status(400).json({ message: '아이디 또는 닉네임에 부적절한 단어가 포함되어 있습니다.' });
    ...
    app.post('/api/auth/login', async (req, res) => {
        ...
        try {
            ...
            else if (!user.baseStats ||
                Object.keys(user.baseStats).length !== Object.keys(defaultBaseStats).length ||
                !Object.values(types.CoreStat).every(stat => user.baseStats[stat] === 100)) {
                user.baseStats = defaultBaseStats;
                await db.updateUser(user);
            }
            ...
            const activeGame = allActiveGames.find(g => (g.player1.id === user.id || g.player2.id === user.id));
            if (activeGame) {
