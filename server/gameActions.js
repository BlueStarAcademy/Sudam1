...
export const resetAndGenerateQuests = async (user) => {
    const now = Date.now();
    ...
    if (isDifferentDayKST(updatedUser.quests.daily?.lastReset, now)) {
        ...
        const newQuests = DAILY_QUESTS.map((q, i) => ({
    ...
    if (isDifferentWeekKST(updatedUser.quests.weekly?.lastReset, now)) {
        ...
        const newQuests = WEEKLY_QUESTS.map((q, i) => ({
    ...
    if (isDifferentMonthKST(updatedUser.quests.monthly?.lastReset, now)) {
        ...
        const newQuests = MONTHLY_QUESTS.map((q, i) => ({
            ...q, id: `q-m-${i}-${now}`, progress: 0, isClaimed: false,
        }));
        updatedUser.quests.monthly.quests = newQuests;
        modified = true;
    }
    const tournamentTypes = ['neighborhood', 'national', 'world'];
    for (const type of tournamentTypes) {
        const playedDateKey = `last${type.charAt(0).toUpperCase() + type.slice(1)}PlayedDate`;
        const rewardClaimedKey = `${type}RewardClaimed`;
        const tournamentKey = `last${type.charAt(0).toUpperCase() + type.slice(1)}Tournament`;
        if (isDifferentDayKST(user[playedDateKey], now)) {
            updatedUser[playedDateKey] = undefined;
            updatedUser[rewardClaimedKey] = undefined;
            updatedUser[tournamentKey] = null;
            modified = true;
        }
    }
    return modified ? updatedUser : user;
};
export const updateQuestProgress = (user, type, mode, amount = 1) => {
    if (!user.quests)
        return;
    ...
    for (const quest of questsToUpdate) {
        ...
        switch (quest.title) {
            case '출석하기':
                ...
                break;
            case '채팅창에 인사하기':
                if (type === 'chat_greeting')
                    shouldUpdate = true;
                break;
            case '전략바둑 플레이하기':
                if (type === 'participate' && isStrategic)
                    shouldUpdate = true;
                break;
            case '놀이바둑 플레이하기':
                if (type === 'participate' && isPlayful)
                    shouldUpdate = true;
                break;
            case '전략바둑 승리하기':
                if (type === 'win' && isStrategic)
                    shouldUpdate = true;
                break;
            case '놀이바둑 승리하기':
                if (type === 'win' && isPlayful)
                    shouldUpdate = true;
                break;
            case '액션버튼 사용하기':
            ...
            case '자동대국 토너먼트 완료하기':
                ...
                break;
            case '자동대국 토너먼트 참여하기':
                if (type === 'tournament_participate')
                    shouldUpdate = true;
                break;
            case '장비 강화시도':
                if (type === 'enhancement_attempt')
                    shouldUpdate = true;
                break;
            case '재료 합성시도':
                if (type === 'craft_attempt')
                    shouldUpdate = true;
                break;
            case '일일퀘스트 활약도100보상 받기(3/3)':
                if (type === 'claim_daily_milestone_100')
                    shouldUpdate = true;
                break;
            ...
            case '주간퀘스트 활약도100보상 받기(2/2)':
                if (type === 'claim_weekly_milestone_100')
                    shouldUpdate = true;
                break;
        }
        if (shouldUpdate) {
...
export const handleAction = async (volatileState, action) => {
    ...
    if (type.startsWith('ADMIN_'))
        return handleAdminAction(volatileState, action, user);
    if (type.includes('NEGOTIATION') || type === 'START_AI_GAME' || type === 'REQUEST_REMATCH' || type === 'CHALLENGE_USER' || type === 'SEND_CHALLENGE')
        return handleNegotiationAction(volatileState, action, user);
    if (type.startsWith('CLAIM_') || type.startsWith('DELETE_MAIL'))
        return handleRewardAction(volatileState, action, user);
    if (type.startsWith('BUY_') || type === 'PURCHASE_ACTION_POINTS' || type === 'EXPAND_INVENTORY')
        return handleShopAction(volatileState, action, user);
    if (type.startsWith('TOURNAMENT') || type.startsWith('START_TOURNAMENT') || type.startsWith('SKIP_TOURNAMENT') || type.startsWith('FORFEIT_TOURNAMENT') || type.startsWith('SAVE_TOURNAMENT') || type.startsWith('CLEAR_TOURNAMENT'))
        return handleTournamentAction(volatileState, action, user);
    if (['TOGGLE_EQUIP_ITEM', 'SELL_ITEM', 'ENHANCE_ITEM', 'DISASSEMBLE_ITEM', 'USE_ITEM', 'USE_ALL_ITEMS_OF_TYPE', 'CRAFT_MATERIAL', 'COMBINE_ITEMS'].includes(type))
        return handleInventoryAction(volatileState, action, user);
    if (['UPDATE_AVATAR', 'UPDATE_BORDER', 'CHANGE_NICKNAME', 'RESET_STAT_POINTS', 'CONFIRM_STAT_ALLOCATION', 'UPDATE_MBTI', 'SAVE_PRESET', 'APPLY_PRESET'].includes(type))
        return handleUserAction(volatileState, action, user);
    if (type.includes('SINGLE_PLAYER'))
