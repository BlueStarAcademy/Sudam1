    ...
    switch (type) {
        case 'CLAIM_MAIL_ATTACHMENTS': {
            ...
            if (mail.attachmentsClaimed)
                return { error: 'Attachments already claimed.' };
            if (!mail.attachments)
                return { error: 'No attachments to claim.' };
            const itemsToCreate = [];
            if (mail.attachments.items) {
                const createdItems = createItemInstancesFromReward(mail.attachments.items);
        ...
        case 'CLAIM_QUEST_REWARD': {
            ...
            const questCategories = ['daily', 'weekly', 'monthly'];
            let foundQuest;
            let questType;
            ...
            if (!foundQuest)
                return { error: '퀘스트를 찾을 수 없습니다.' };
            if (foundQuest.isClaimed)
                return { error: '이미 보상을 수령했습니다.' };
            if (foundQuest.progress < foundQuest.target)
                return { error: '퀘스트를 아직 완료하지 않았습니다.' };
            const { reward, activityPoints } = foundQuest;
            const itemsToCreate = [];
            if (reward.items) {
                const createdItems = createItemInstancesFromReward(reward.items);
                itemsToCreate.push(...createdItems);
            }
            const { success, finalItemsToAdd } = addItemsToInventory([...user.inventory], user.inventorySlots, itemsToCreate);
            if (!success) {
        ...
        case 'CLAIM_ACTIVITY_MILESTONE': {
            ...
            if (!selectedQuest || !selectedQuest.data)
                return { error: "유효하지 않은 퀘스트 타입입니다." };
            const { data, thresholds, rewards } = selectedQuest;
            if (milestoneIndex < 0 || milestoneIndex >= rewards.length)
                return { error: "유효하지 않은 마일스톤입니다." };
            if (data.claimedMilestones[milestoneIndex])
                return { error: "이미 수령한 보상입니다." };
            const requiredProgress = thresholds[milestoneIndex];
            if (data.activityProgress < requiredProgress)
                return { error: "활약도 점수가 부족합니다." };
        ...
        case 'CLAIM_TOURNAMENT_REWARD': {
            ...
            if (user[statusKey])
                return { error: '이미 보상을 수령했습니다.' };
            const tournamentState = user[tourneyKey];
            ...
            if (tournamentType === 'neighborhood')
                itemRewardKey = userRank <= 3 ? userRank : 4;
            else if (tournamentType === 'national')
                itemRewardKey = userRank <= 4 ? userRank : 5;
            else { // world
                if (userRank <= 4)
                    itemRewardKey = userRank;
                else if (userRank <= 8)
                    itemRewardKey = 5;
                else
                    itemRewardKey = 9;
            ...
            const itemsToCreate = itemReward.items ? createItemInstancesFromReward(itemReward.items) : [];
            const { success, finalItemsToAdd } = addItemsToInventory([...user.inventory], user.inventorySlots, itemsToCreate);
            if (!success) {
            ...
            if (itemReward.gold)
                allObtainedItems.unshift({ name: `${itemReward.gold} 골드`, image: '/images/icon/Gold.png' });
            if (itemReward.diamonds)
                allObtainedItems.unshift({ name: `${itemReward.diamonds} 다이아`, image: '/images/icon/Zem.png' });
