...
export const handleShopAction = async (volatileState, action, user) => {
    ...
    switch (type) {
        ...
        case 'BUY_BORDER': {
            ...
            if (!borderItem)
                return { error: '판매하지 않는 테두리입니다.' };
            if (user.ownedBorders.includes(borderId))
                return { error: '이미 보유한 테두리입니다.' };
            ...
            if (user.diamonds < diamondCost && !user.isAdmin)
                return { error: '다이아가 부족합니다.' };
            if (!user.isAdmin) {
                user.gold -= cost;
