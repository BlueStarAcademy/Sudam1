...
import { SpecialStat, MythicStat } from '../../types.js';
import { EQUIPMENT_POOL, MAIN_STAT_DEFINITIONS, SPECIAL_STATS_DATA, MYTHIC_STATS_DATA, ENHANCEMENT_SUCCESS_RATES, ENHANCEMENT_COSTS, MATERIAL_ITEMS, ITEM_SELL_PRICES, MATERIAL_SELL_PRICES, SUB_OPTION_POOLS, GRADE_SUB_OPTION_RULES, GRADE_LEVEL_REQUIREMENTS, ENHANCEMENT_FAIL_BONUS_RATES, BLACKSMITH_COMBINATION_GREAT_SUCCESS_RATES, BLACKSMITH_COMBINATION_XP_GAIN, BLACKSMITH_MAX_LEVEL, BLACKSMITH_XP_REQUIRED_FOR_LEVEL_UP, BLACKSMITH_COMBINABLE_GRADES_BY_LEVEL, calculateEnhancementGoldCost } from '../../constants.js';
import { BLACKSMITH_ENHANCEMENT_XP_GAIN, BLACKSMITH_DISASSEMBLY_XP_GAIN, BLACKSMITH_DISASSEMBLY_JACKPOT_RATES, } from '../../constants/rules.js';
import * as effectService from '../effectService.js';
import { SHOP_ITEMS } from '../shop.js';
import { updateQuestProgress } from '../questService.js';
import { addItemsToInventory as addItemsToInventoryUtil } from '../../utils/inventoryUtils.js';
const ALL_SLOTS = ['fan', 'board', 'top', 'bottom', 'bowl', 'stones'];
const GRADE_ORDER = ['normal', 'uncommon', 'rare', 'epic', 'legendary', 'mythic'];
...
const generateNewItem = (grade, slot) => {
    ...
    const existingSubTypes = new Set([mainOption.type]);
...
const removeUserItems = (user, itemsToRemove) => {
...
export const handleInventoryAction = async (volatileState, action, user) => {
    const { type, payload } = action;
    switch (type) {
        case 'COMBINE_ITEMS': {
            const { itemIds, isRandom } = payload;
            if (!itemIds || itemIds.length !== 3)
                return { error: '합성에는 3개의 아이템이 필요합니다.' };
            const itemsToCombine = user.inventory.filter(i => itemIds.includes(i.id));
            if (itemsToCombine.length !== 3)
                return { error: '선택된 아이템 중 일부를 찾을 수 없습니다.' };
            const firstItem = itemsToCombine[0];
            const grade = firstItem.grade;
            // Validation
            if (itemsToCombine.some(i => i.isEquipped))
                return { error: '장착 중인 아이템은 합성할 수 없습니다.' };
            if (itemsToCombine.some(i => i.grade !== grade))
                return { error: '같은 등급의 아이템만 합성할 수 있습니다.' };
            ...
            if (isRandom) {
                ...
                const weightedSlots = itemsToCombine.map(i => i.slot).filter((s) => s !== null);
        ...
        case 'USE_ITEM': {
            ...
            const shopItemKey = Object.keys(SHOP_ITEMS).find(key => SHOP_ITEMS[key].name === item.name);
            if (!shopItemKey)
        ...
        case 'USE_ALL_ITEMS_OF_TYPE': {
            ...
            if (totalGoldGained > 0)
                clientResponseItems.push({ name: '골드', quantity: totalGoldGained, image: '/images/icon/Gold.png', type: 'material', grade: 'uncommon' });
            if (totalDiamondsGained > 0)
                clientResponseItems.push({ name: '다이아', quantity: totalDiamondsGained, image: '/images/icon/Zem.png', type: 'material', grade: 'rare' });
        ...
        case 'TOGGLE_EQUIP_ITEM': {
            ...
            if (itemToToggle.isEquipped) {
                ...
                const currentItemInSlot = user.inventory.find(i => i.isEquipped && i.slot === itemToToggle.slot);
        ...
        case 'ENHANCE_ITEM': {
            ...
            if (!item || item.type !== 'equipment' || !item.options)
                return { error: '강화할 수 없는 아이템입니다.' };
            if (item.stars >= 10)
                return { error: '최대 강화 레벨입니다.' };
            const targetStars = item.stars + 1;
            const userLevelSum = user.strategyLevel + user.playfulLevel;
            const enhancementLevelRequirements = {
                4: 3,
                7: 8,
                10: 15,
            };
        ...
        case 'DISASSEMBLE_ITEM': {
            ...
            for (const itemId of itemIds) {
                ...
                if (item.type !== 'equipment')
                    return { error: '장비 아이템만 분해할 수 있습니다.' };
                if (item.isEquipped)
                    return { error: '장착 중인 아이템은 분해할 수 없습니다.' };
            ...
            if (itemsToRemove.length === 0)
                return { error: '분해할 수 있는 아이템이 없습니다.' };
            ...
            const itemsToAdd = Object.entries(gainedMaterials).map(([name, quantity]) => ({
            ...
            if (!success)
                return { error: '재료를 받기에 인벤토리 공간이 부족합니다.' };
        ...
        case 'CRAFT_MATERIAL': {
            const { materialName, craftType, quantity } = payload;
            const materialTiers = ['하급 강화석', '중급 강화석', '상급 강화석', '최상급 강화석', '신비의 강화석'];
            const tierIndex = materialTiers.indexOf(materialName);
            if (tierIndex === -1)
                return { error: '잘못된 재료입니다.' };
            let fromMaterialName, toMaterialName, fromCost, toYield;
            if (craftType === 'upgrade') {
                if (tierIndex >= materialTiers.length - 1)
                    return { error: '더 이상 제작할 수 없습니다.' };
        ...
        case 'EXPAND_INVENTORY': {
            ...
            if (!category)
                return { error: '확장할 인벤토리 탭을 지정해야 합니다.' };
