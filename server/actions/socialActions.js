...
export const handleSocialAction = async (volatileState, action, user) => {
    ...
    switch (type) {
        case 'LOGOUT': {
            ...
            if (activeTournament) {
                ...
                user[stateKey] = activeTournament;
                    ...
                    if (!game.disconnectionState) {
                        ...
                        if (game.disconnectionCounts[user.id] >= 3) {
                            ...
                            if (game.moveHistory.length < 10) {
                                ...
                                if (!game.canRequestNoContest)
                                    game.canRequestNoContest = {};
        ...
        case 'SEND_CHAT_MESSAGE': {
            ...
            if (messageContent) {
                if (!volatileState.userConsecutiveChatMessages)
                    volatileState.userConsecutiveChatMessages = {};
                ...
                if (volatileState.userConsecutiveChatMessages[user.id].count >= 3 && !user.isAdmin) {
                    ...
                    if (!volatileState.waitingRoomChats[channel])
                        volatileState.waitingRoomChats[channel] = [];
        ...
        case 'LEAVE_GAME_ROOM': {
            ...
            const ongoingRematchNegotiation = Object.values(volatileState.negotiations).find(neg => neg.rematchOfGameId === gameId);
