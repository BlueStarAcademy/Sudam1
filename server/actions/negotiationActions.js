...
export const handleNegotiationAction = async (volatileState, action, user) => {
    ...
    switch (type) {
        case 'CHALLENGE_USER': {
            ...
            if (opponentId !== aiUserId) {
                ...
                const isOpponentInPendingNegotiation = Object.values(volatileState.negotiations).some(neg => (neg.opponent.id === opponent.id || neg.challenger.id === opponent.id) && neg.status === 'pending');
        ...
        case 'SEND_CHALLENGE': {
            ...
            const isOpponentAlreadyInNegotiation = Object.values(volatileState.negotiations).some(neg => neg.id !== negotiationId &&
        ...
        case 'ACCEPT_NEGOTIATION': {
            ...
            if (!challenger || !opponent)
                return { error: "One of the players could not be found." };
            const cost = getActionPointCost(negotiation.mode);
        ...
        case 'START_AI_GAME': {
            ...
            if (draftNegId)
                delete volatileState.negotiations[draftNegId];
