...
const getPhase = (time) => {
    if (time <= EARLY_GAME_DURATION)
        return 'early';
    if (time <= EARLY_GAME_DURATION + MID_GAME_DURATION)
        return 'mid';
    return 'end';
};
const calculatePower = (player, phase) => {
...
const finishMatch = (match, p1, p2, p1Cumulative, p2Cumulative) => {
...
const simulateAndFinishMatch = (match, players) => {
    if (match.isFinished)
        return;
...
const prepareNextRound = (state, user) => {
    ...
    if (lastRound.matches.every(m => m.isFinished)) {
        ...
        if (winners.length > 1) {
            if (winners.length === 2 && state.players.length === 8 && lastRound.matches.length === 2) {
                const losers = lastRound.matches.map(m => m.players.find(p => p && p.id !== m.winner?.id)).filter(Boolean);
                if (losers.length === 2) {
...
const processMatchCompletion = (state, user, completedMatch, roundIndex) => {
    ...
    if (state.type === 'neighborhood') {
        ...
        const roundMatches = roundPairings.map(pair => {
            ...
            return state.rounds[0].matches.find(m => (m.players[0]?.id === p1Id && m.players[1]?.id === p2Id) ||
...
export const createTournament = (type, user, players) => {
...
export const startNextRound = (state, user) => {
...
export const skipToResults = (state, userId) => {
    ...
    while (state.status !== 'complete' && safety < 10) {
        ...
        prepareNextRound(state, { id: userId });
...
export const forfeitTournament = (state, userId) => {
...
export const advanceSimulation = (state, user) => {
    ...
    if (state.timeElapsed > 1 && state.timeElapsed < TOTAL_GAME_DURATION && state.timeElapsed % 5 === 0) {
        ...
        if (Math.random() < 0.20) { // Base 20% chance for any event
            ...
            let highStatPlayer, lowStatPlayer;
            if (p1Stat > p2Stat) {
                highStatPlayer = p1;
                lowStatPlayer = p2;
            }
...
export const calculateRanks = (tournament) => {
                ...
                if (match.isFinished && match.winner && match.players[0] && match.players[1]) {
                    ...
                    if (!rankedPlayerIds.has(loser.id)) {
                        ...
                        if (round.name.includes("강"))
                            rank = parseInt(round.name.replace("강", ""));
                        else if (round.name.includes("결승"))
                            rank = 2;
                        else if (round.name.includes("3,4위전"))
                            rank = 4;
