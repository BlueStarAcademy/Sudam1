...
import { RANKING_TIERS, SEASONAL_TIER_REWARDS, LEAGUE_DATA, LEAGUE_WEEKLY_REWARDS, SPECIAL_GAME_MODES, PLAYFUL_GAME_MODES, SEASONAL_TIER_BORDERS } from '../constants.js';
import { randomUUID } from 'crypto';
import { getKSTDate, getCurrentSeason, getPreviousSeason, isDifferentWeekKST } from '../utils/timeUtils.js';
let lastSeasonProcessed = null;
const processRewardsForSeason = async (season) => {
    ...
    for (const user of allUsers) {
        ...
        for (const mode of allGameModes) {
            ...
            if (!user.seasonHistory)
                user.seasonHistory = {};
            if (!user.seasonHistory[season.name])
                user.seasonHistory[season.name] = {};
        ...
        if (bestTierInfo) {
            ...
            if (seasonalBorderId) {
                if (!user.ownedBorders)
                    user.ownedBorders = ['default', 'simple_black']; // Ensure array exists
...
export const processRankingRewards = async (volatileState) => {
    const now = Date.now();
    const kstNow = getKSTDate(now);
    // Check if it's the start of a new season day
    const isNewSeasonDay = (kstNow.getUTCMonth() === 0 && kstNow.getUTCDate() === 1) || // Jan 1
    ...
    if (lastSeasonProcessed === null) {
        const saved = await db.getKV('lastSeasonProcessed');
        if (saved) {
...
export async function processWeeklyLeagueUpdates(user) {
...
export async function updateWeeklyCompetitorsIfNeeded(user, allUsers) {
    const now = Date.now();
    ...
    const potentialCompetitors = allUsers.filter(u => u.id !== user.id && u.league === user.league);
    ...
    const competitorList = [user, ...selectedCompetitors].map(u => ({
        id: u.id,
        nickname: u.nickname,
        avatarId: u.avatarId,
        borderId: u.borderId,
