...
import { ALKKAGI_TURN_TIME_LIMIT, CURLING_TURN_TIME_LIMIT, DICE_GO_MAIN_ROLL_TIME } from '../../constants.js';
import * as effectService from '../effectService.js';
import { aiUserId } from '../aiPlayer.js';
import { randomUUID } from 'crypto';
export const initializePlayfulGame = async (game, neg, now) => {
...
export const updatePlayfulGameState = async (game, now) => {
    switch (game.mode) {
        case types.GameMode.Dice:
    ...
    if (playableStatuses.includes(game.gameStatus)) {
        ...
        for (const player of players) {
            if (!player || player.id.startsWith('bot-') || player.id === aiUserId)
                continue;
...
export const handlePlayfulGameAction = async (volatileState, game, action, user) => {
    switch (game.mode) {
        case types.GameMode.Dice:
