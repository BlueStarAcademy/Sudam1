...
export const initializeBase = (game, now) => {
...
const placeRemainingStonesRandomly = (game, playerKey) => {
...
const resolveBasePlacementAndTransition = (game, now) => {
    const target = game.settings.baseStones ?? 4;
    ...
    const overlappingCoords = new Set();
    ...
    if (validP1Stones.length > 0 || validP2Stones.length > 0) {
        const tempBoard = Array(boardSize).fill(0).map(() => Array(boardSize).fill(types.Player.None));
        validP1Stones.forEach(p => tempBoard[p.y][p.x] = types.Player.Black);
        validP2Stones.forEach(p => tempBoard[p.y][p.x] = types.Player.White);
        const tempGame = { boardState: tempBoard, settings: { boardSize } };
...
export const updateBaseState = (game, now) => {
    ...
    switch (game.gameStatus) {
        ...
        case 'komi_bidding': {
            ...
            if (bothHaveBid || deadlinePassed) {
                if (deadlinePassed) {
                    ...
                    if (!game.komiBids[p1Id])
                        game.komiBids[p1Id] = timeoutBid;
                    if (!game.komiBids[p2Id])
                        game.komiBids[p2Id] = timeoutBid;
                }
        ...
        case 'komi_bid_reveal':
            if (game.revealEndTime && now > game.revealEndTime && !game.komiBidRevealProcessed) {
                ...
                let blackPlayerId, whitePlayerId, finalKomi;
...
export const handleBaseAction = (game, action, user) => {
    const { type, payload } = action;
    const now = Date.now();
    switch (type) {
        case 'PLACE_BASE_STONE':
            if (game.gameStatus !== 'base_placement')
                return { error: "Not in base placement phase." };
            const myStonesKey = user.id === game.player1.id ? 'baseStones_p1' : 'baseStones_p2';
            if (!game[myStonesKey])
                game[myStonesKey] = [];
            if ((game[myStonesKey]?.length ?? 0) >= game.settings.baseStones)
                return { error: "Already placed all stones." };
            if (game[myStonesKey].some(p => p.x === payload.x && p.y === payload.y))
                return { error: "Already placed a stone there." };
        ...
        case 'UPDATE_KOMI_BID':
            if (game.gameStatus !== 'komi_bidding' || game.komiBids?.[user.id])
                return { error: "Cannot bid now." };
            if (!game.komiBids)
                game.komiBids = {};
