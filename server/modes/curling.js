...
const runServerSimulation = (initialStones, flickedStone, velocity) => {
    let simStones = JSON.parse(JSON.stringify(initialStones || []));
...
const endCurlingRound = (game, now) => {
    const boardSizePx = 840;
    ...
    for (const stone of onBoardStones) {
        ...
        if (dist <= cellSize * 0.5)
            score = 5;
        else if (dist <= cellSize * 2)
            score = 3;
        else if (dist <= cellSize * 4)
            score = 2;
        else if (dist <= cellSize * 6)
            score = 1;
        if (score > 0) {
            scoredStones[stone.id] = score;
            if (stone.player === types.Player.Black)
                houseScoreBlack += score;
            else
                houseScoreWhite += score;
        }
    }
...
export const initializeCurling = (game, neg, now) => {
...
export const updateCurlingState = (game, now) => {
    ...
    switch (game.gameStatus) {
        ...
        case 'curling_playing': {
            if (game.curlingTurnDeadline && now > game.curlingTurnDeadline) {
                ...
                if (gameEnded)
                    return;
                if (!game.curlingStonesLostToFoul)
                    game.curlingStonesLostToFoul = {};
                ...
                const bothPlayersHaveThrownAllStones = game.stonesThrownThisRound[p1Id] >= game.settings.curlingStoneCount &&
        ...
        case 'curling_animating': {
            if (game.animation && now > game.animation.startTime + game.animation.duration) {
                const { stone, velocity } = game.animation;
...
export const handleCurlingAction = async (volatileState, game, action, user) => {
    ...
    switch (type) {
        case 'CONFIRM_CURLING_START': {
            if (game.mode !== types.GameMode.Curling)
                return undefined;
            if (game.gameStatus !== 'curling_start_confirmation')
        ...
        case 'CURLING_FLICK_STONE': {
            if (game.gameStatus !== 'curling_playing' || !isMyTurn)
        ...
        case 'USE_CURLING_ITEM': {
            ...
            if (game.activeCurlingItems?.[user.id]?.includes(itemType))
        ...
        case 'CONFIRM_ROUND_END': {
            if (game.gameStatus !== 'curling_round_end')
