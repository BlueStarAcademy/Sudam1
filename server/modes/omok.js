...
export const initializeOmok = (game, neg, now) => {
    if (game.isAiGame) {
...
export const updateOmokState = (game, now) => {
...
export const handleOmokAction = async (volatileState, game, action, user) => {
    ...
    switch (type) {
        case 'OMOK_PLACE_STONE': {
            if (game.gameStatus !== 'playing')
                return { error: "Cannot place stone now." };
            if (!isMyTurn)
