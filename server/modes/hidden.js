...
export const initializeHidden = (game) => {
...
export const updateHiddenState = (game, now) => {
...
export const handleHiddenAction = (volatileState, game, action, user) => {
    ...
    switch (type) {
        ...
        case 'SCAN_BOARD':
            if (game.gameStatus !== 'scanning')
                return { error: "Not in scanning mode." };
            const { x, y } = payload;
            ...
            if (success) {
                ...
                if (!game.revealedHiddenMoves[user.id])
                    game.revealedHiddenMoves[user.id] = [];
