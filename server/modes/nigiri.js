...
export const initializeNigiri = (game, now) => {
...
export const updateNigiriState = (game, now) => {
    switch (game.gameStatus) {
        ...
        case 'nigiri_guessing':
            if (game.guessDeadline && now > game.guessDeadline && game.nigiri && game.nigiri.guess === null) {
                const randomGuess = (Math.random() < 0.5 ? 1 : 2);
...
export const handleNigiriAction = (game, action, user) => {
    const { type, payload } = action; // Cast to any to access payload without TS complaining before the switch
    const now = Date.now();
