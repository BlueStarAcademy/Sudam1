...
export const initializeStrategicGame = (game, neg, now) => {
...
export const updateStrategicGameState = async (game, now) => {
...
export const handleStrategicGameAction = async (volatileState, game, action, user) => {
...
const handleStandardAction = async (volatileState, game, action, user) => {
    ...
    switch (type) {
        case 'PLACE_STONE': {
            ...
            const isTargetHiddenOpponentStone = stoneAtTarget === opponentPlayerEnum &&
                moveIndexAtTarget !== -1 &&
            ...
            if (result.capturedStones.length > 0) {
                ...
                for (const captured of result.capturedStones) {
                    ...
                    for (const n of neighbors) {
                        ...
                        if (checkedStones.has(neighborKey) || boardAfterMove[n.y][n.x] !== myPlayerEnum)
                            continue;
