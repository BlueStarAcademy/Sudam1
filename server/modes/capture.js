...
export const initializeCapture = (game, now) => {
...
export const updateCaptureState = (game, now) => {
    const p1Id = game.player1.id;
    const p2Id = game.player2.id;
        ...
        case 'capture_bidding': {
            ...
            if (bothHaveBid || deadlinePassedBid) {
                if (deadlinePassedBid) {
                    if (game.bids[p1Id] == null)
                        game.bids[p1Id] = 1;
                    if (game.bids[p2Id] == null)
                        game.bids[p2Id] = 1;
                }
...
export const handleCaptureAction = (game, action, user) => {
    const { type, payload } = action;
    switch (type) {
        case 'UPDATE_CAPTURE_BID':
            if (game.gameStatus !== 'capture_bidding' || game.bids?.[user.id])
