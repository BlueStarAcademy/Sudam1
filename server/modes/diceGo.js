...
function finishPlacingTurn(game, playerId) {
    const now = Date.now();
...
const handleTimeoutFoul = (game, timedOutPlayerId, now) => {
...
export const initializeDiceGo = (game, neg, now) => {
    ...
    const findGroupSize = (startX, startY) => {
            ...
            if (shouldExtend) {
                ...
                for (let y = 0; y < boardSize; y++)
                    for (let x = 0; x < boardSize; x++)
                        if (tempBoard[y][x] === types.Player.White)
...
export const updateDiceGoState = (game, now) => {
    ...
    switch (game.gameStatus) {
        case 'dice_turn_rolling':
            ...
            if (game.turnOrderRollDeadline && now > game.turnOrderRollDeadline) {
                if (!game.turnOrderRollReady?.[p1Id])
                    game.turnOrderRollReady[p1Id] = true;
                if (!game.turnOrderRollReady?.[p2Id])
                    game.turnOrderRollReady[p2Id] = true;
            }
...
export const handleDiceGoAction = async (volatileState, game, action, user) => {
    ...
    switch (type) {
        case 'DICE_READY_FOR_TURN_ROLL': {
            if (game.gameStatus !== 'dice_turn_rolling')
                return { error: 'Not in turn rolling phase.' };
            if (!game.turnOrderRollReady)
                game.turnOrderRollReady = {};
            game.turnOrderRollReady[user.id] = true;
            await db.saveGame(game);
            return {};
        }
        case 'DICE_CHOOSE_TURN': {
            if (game.gameStatus !== 'dice_turn_choice' || user.id !== game.turnChooserId)
                return { error: 'Not your turn to choose.' };
            const { choice } = payload;
            const chooserId = game.turnChooserId;
            if (!chooserId)
                return { error: 'Chooser not set.' };
        ...
        case 'DICE_CONFIRM_START': {
            if (game.gameStatus !== 'dice_start_confirmation')
        ...
        case 'DICE_ROLL': {
            if (game.gameStatus !== 'dice_rolling' || !isMyTurn)
        ...
        case 'DICE_PLACE_STONE': {
            if (game.gameStatus !== 'dice_placing' || !isMyTurn)
        ...
        case 'CONFIRM_ROUND_END': {
            if (game.gameStatus !== 'dice_round_end')
                return { error: "Not in round end confirmation phase." };
            if (!game.roundEndConfirmations)
                game.roundEndConfirmations = {};
