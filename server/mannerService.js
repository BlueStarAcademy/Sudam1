...
export const getMannerScore = (user) => {
    return user.mannerScore ?? 200;
};
export const getMannerRank = (score) => {
    if (score >= 2000)
        return { rank: '마스터', color: 'text-purple-400' };
    if (score >= 1600)
        return { rank: '프로', color: 'text-blue-400' };
    if (score >= 1100)
        return { rank: '품격', color: 'text-cyan-400' };
    if (score >= 800)
        return { rank: '매우 좋음', color: 'text-teal-400' };
    if (score >= 400)
        return { rank: '좋음', color: 'text-green-400' };
    if (score >= 200)
        return { rank: '보통', color: 'text-gray-300' };
    if (score >= 100)
        return { rank: '주의', color: 'text-yellow-400' };
    if (score >= 50)
        return { rank: '나쁨', color: 'text-orange-400' };
    if (score >= 1)
        return { rank: '매우 나쁨', color: 'text-red-500' };
    return { rank: '최악', color: 'text-red-700' };
};
export const getMannerStyle = (score) => {
...
export const applyMannerRankChange = async (user, oldMannerScore) => {
    ...
    const regeneratedUser = await regenerateActionPoints(tempUserWithOldScore);
    ...
    if (newRank === '마스터' && oldRank !== '마스터' && !user.mannerMasteryApplied) {
        ...
        const statKeys = Object.keys(spentStats);
