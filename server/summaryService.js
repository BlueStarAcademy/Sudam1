...
import { Player, GameMode, SpecialStat } from '../types.js';
import * as db from './db.js';
import { SPECIAL_GAME_MODES, CONSUMABLE_ITEMS, PLAYFUL_GAME_MODES, SINGLE_PLAYER_STAGES } from '../constants.js';
...
const getXpForLevel = (level) => 1000 + (level - 1) * 200;
const processSinglePlayerGameSummary = async (game) => {
...
export const endGame = async (game, winner, winReason) => {
...
const createConsumableItemInstance = (name) => {
...
// Playful Loot Tables
const PLAYFUL_LOOT_TABLE_3_ROUNDS = [
...
const PLAYFUL_LOOT_TABLE_2_ROUNDS = [
...
const PLAYFUL_LOOT_TABLE_1_ROUND = [
...
// Strategic Gold Map
const STRATEGIC_GOLD_REWARDS = {
    19: 1500, 17: 1300, 15: 1100, 13: 900, 11: 700, 9: 500, 7: 300
};
// Playful Gold Map
const PLAYFUL_GOLD_REWARDS = {
    3: 800, 2: 500, 1: 200,
};
// --- END NEW REWARD CONSTANTS ---
const calculateGameRewards = (game, player, isWinner, isDraw, itemDropBonus, materialDropBonus, rewardMultiplier) => {
    const { mode, settings, isAiGame } = game;
    let baseGold = 0;
    let lootTable = [];
    if (SPECIAL_GAME_MODES.some(m => m.mode === mode)) {
        baseGold = STRATEGIC_GOLD_REWARDS[settings.boardSize] || STRATEGIC_GOLD_REWARDS[19];
        lootTable = STRATEGIC_LOOT_TABLE;
    }
    else {
        let rounds = 1; // Default for Omok/Ttamok
        if (mode === GameMode.Dice)
            rounds = settings.diceGoRounds || 3;
        else if (mode === GameMode.Alkkagi)
            rounds = settings.alkkagiRounds || 1;
        else if (mode === GameMode.Curling)
            rounds = settings.curlingRounds || 3;
        else if (mode === GameMode.Thief)
            rounds = 2;
        baseGold = PLAYFUL_GOLD_REWARDS[rounds] || PLAYFUL_GOLD_REWARDS[1];
        if (rounds === 3)
            lootTable = PLAYFUL_LOOT_TABLE_3_ROUNDS;
        else if (rounds === 2)
            lootTable = PLAYFUL_LOOT_TABLE_2_ROUNDS;
        else
            lootTable = PLAYFUL_LOOT_TABLE_1_ROUND;
    }
...
const calculateEloChange = (playerRating, opponentRating, result) => {
    const K = 32;
...
const processPlayerSummary = async (player, opponent, isWinner, isDraw, game, isNoContest, isInitiator) => {
    const updatedPlayer = JSON.parse(JSON.stringify(player)); // Create a deep mutable copy
    ...
    const xpSummary = { initial: initialXp, change: xpGain, final: currentXp };
    const levelSummary = {
    ...
    const ratingSummary = { initial: initialRating, change: ratingChange, final: gameStats.rankingScore };
    // --- Manner Score ---
...
export const processGameSummary = async (game) => {
    ...
    if (!game.summary)
        game.summary = {}; // Initialize summary object
    try {
        if (p1.id !== aiUserId) {
