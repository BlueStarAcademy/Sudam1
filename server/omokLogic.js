...
export const getOmokLogic = (game) => {
    const { settings: { boardSize } } = game;
    const player = game.currentPlayer;
    const opponent = player === types.Player.Black ? types.Player.White : types.Player.Black;
    const getLine = (x, y, dx, dy, board) => {
        const line = [{ x, y }];
        const p = board[y][x];
        if (p === types.Player.None)
            return [];
        for (let i = 1; i < 6; i++) {
            const nx = x + i * dx;
            const ny = y + i * dy;
            if (nx < 0 || nx >= boardSize || ny < 0 || ny >= boardSize || board[ny][nx] !== p)
                break;
            line.push({ x: nx, y: ny });
        }
        for (let i = 1; i < 6; i++) {
    ...
    const checkOpenState = (line) => {
        let openEnds = 0;
        if (line[0] === types.Player.None)
            openEnds++;
        if (line[line.length - 1] === types.Player.None)
            openEnds++;
        return openEnds;
    };
    const is33 = (x, y, board) => {
        // The original logic was buggy and preventing valid moves.
        // As per the request, this check is disabled.
        return false;
    };
    const performTtamokCapture = (x, y) => {
        let capturedCount = 0;
        const directions = [{ dx: 1, dy: 0 }, { dx: 0, dy: 1 }, { dx: 1, dy: 1 }, { dx: 1, dy: -1 }];
        for (const { dx, dy } of directions) {
            for (const dir of [1, -1]) { // Check both ways on an axis
                const n1x = x + dir * dx;
                const n1y = y + dir * dy;
                const n2x = x + dir * 2 * dx;
                const n2y = y + dir * 2 * dy;
                const n3x = x + dir * 3 * dx;
                const n3y = y + dir * 3 * dy;
                if (n3x < 0 || n3x >= boardSize || n3y < 0 || n3y >= boardSize)
                    continue;
                if (game.boardState[n1y]?.[n1x] === opponent &&
    ...
    const checkPotentialCaptures = (x, y, player, board) => {
        let capturedCount = 0;
        const opponent = player === types.Player.Black ? types.Player.White : types.Player.Black;
        const directions = [{ dx: 1, dy: 0 }, { dx: 0, dy: 1 }, { dx: 1, dy: 1 }, { dx: 1, dy: -1 }];
    ...
    const getLineStats = (x, y, player, board, dx, dy) => {
        const boardSize = board.length;
        let length = 1;
        let line = [{ x, y }];
        // Forward
