...
export const createUser = async (db: Database, user: User): Promise<void> => {
    await db.run(
        `INSERT INTO users ( 
            ...
            mbti, isMbtiPublic, monthlyGoldBuffExpiresAt, singlePlayerProgress, clearedSinglePlayerStages, bonusStatPoints, blacksmithLevel, blacksmithXp, cumulativeTournamentScore, singlePlayerMissions
        ) 
         VALUES ( 
            ?, ?, ?, ?, ?, ?, ?, ?, 
            ?, ?, ?, ?, ?, ?, ?, 
        ...
        user.mbti, user.isMbtiPublic, user.monthlyGoldBuffExpiresAt, user.singlePlayerProgress, JSON.stringify(user.clearedSinglePlayerStages || []), user.bonusStatPoints, user.blacksmithLevel, user.blacksmithXp, user.cumulativeTournamentScore ?? 0, JSON.stringify(user.singlePlayerMissions || {})
    );
};
