...
export const getMannerEffects = (user) => {
...
export const calculateUserEffects = (user) => {
    ...
    for (const item of equippedItems) {
        ...
        for (const opt of allOptions) {
            ...
            if (Object.values(CoreStat).includes(type)) {
                if (isPercentage) {
                    calculatedEffects.coreStatBonuses[type].percent += value;
                }
                else {
                    calculatedEffects.coreStatBonuses[type].flat += value;
                }
            }
            else if (Object.values(SpecialStat).includes(type)) {
                if (isPercentage) {
                    calculatedEffects.specialStatBonuses[type].percent += value;
                }
                else {
                    calculatedEffects.specialStatBonuses[type].flat += value;
                }
            }
            else if (Object.values(MythicStat).includes(type)) {
                // Mythic stats are generally flat values (e.g., +1 item)
                calculatedEffects.mythicStatBonuses[type].flat += value;
            }
        }
    }
...
export const regenerateActionPoints = async (user) => {
