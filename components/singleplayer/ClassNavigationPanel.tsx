...
const ClassNavigationPanel: React.FC<ClassNavigationPanelProps> = ({ selectedClass, onClassSelect }) => {
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-lg p-4 h-full flex flex-col">
            <h2 className="text-xl font-bold text-gray-200 mb-4 border-b border-gray-700 pb-2 text-center">단계 선택</h2>
            ...
            <div className="flex-1 flex items-center justify-center relative">
                ...
                <button
                    ...
                    className={`
                        absolute left-0 z-10 w-12 h-12 rounded-full bg-gray-700 hover:bg-gray-600
                        ...
                        shadow-lg border-2 border-gray-600
                    `}
                    aria-label="이전 단계"
                >
                    <svg 
                ...
                {/* 현재 선택된 클래스 이미지 */}
                <div className="flex-1 h-full flex flex-col items-center justify-center mx-12 relative">
                    <div className="relative w-full h-full max-h-[500px] rounded-lg overflow-hidden shadow-xl border-4 border-purple-600">
                        <img 
                            ...
                            onError={(e) => {
                                ...
                                if (parent) {
                                    parent.innerHTML = `<div class="w-full h-full flex items-center justify-center bg-gray-700 text-gray-200 font-bold text-2xl">${currentClass.name}</div>`;
                                }
                            }}
                        />
                        {/* 클래스 이름 오버레이 */}
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/70 to-transparent text-white text-center py-4 px-4">
                            <div className="text-2xl lg:text-3xl font-bold mb-1">{currentClass.name}</div>
                ...
                <button
                    ...
                    className={`
                        absolute right-0 z-10 w-12 h-12 rounded-full bg-gray-700 hover:bg-gray-600
                        flex items-center justify-center transition-all duration-200
